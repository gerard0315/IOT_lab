...
class mtaUpdates(object):
	...
	for entity in feed.entity:
		...
		##### INSERT TRIPUPDATE CODE HERE ####			
                ...
                directionIdx = int(update.tripId.rfind('.'))
                update.direction = update.tripId[int(directionIdx+1):int(directionIdx+2)]
                update.time = feed.header.timestamp                 
                update.futureStops = OrderedDict()
                for stop in entity.trip_update.stop_time_update:
                    stopId = stop.stop_id
                    stopDict = OrderedDict()
                    stopDict['arrivalTime'] = stop.arrival.time
                    stopDict['departureTime'] = stop.departure.time
                    update.futureStops[stopId] = stopDict
...
if __name__ == "__main__":
    updateObj = mtaUpdates('20778f9857669c6fdf7fbd4e4f07fd30');
    updateObj.getTripUpdates();
